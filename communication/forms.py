...
class ParentSendMessageForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        user_parent = kwargs.pop('user_parent', None)
        super(ParentSendMessageForm, self).__init__(*args, **kwargs)

        if user_parent:
            student_instance = UserStudent.objects.get(parentUsername=user_parent).student
